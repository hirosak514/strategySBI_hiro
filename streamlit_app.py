...
# --- 2. API設定 ---
current_api_key = st.session_state.api_key or st.secrets.get("gemini_key", "")
if current_api_key:
    genai.configure(api_key=current_api_key)

# --- 3. 価格取得関数 (最適化版) ---
@st.cache_data(ttl=60) # 1分間だけキャッシュ
def get_live_prices(portfolio_keys):
    ...
    for key in portfolio_keys:
        ...
        try:
            ...
            # 最新の1分足データを取得して「今」の価格を優先する
            hist = stock.history(period="1d", interval="1m")
            if not hist.empty:
                prices[key] = {
                    "current": hist['Close'].iloc[-1],
                    "prev_close": hist['Close'].iloc[0] # 本日の始値付近を比較対象に
                }
            else:
                prices[key] = None
        except:
...
def analyze_multiple_images(uploaded_files):
    ...
    prompt = """
    証券口座のスクリーンショットから保有銘柄を抽出し、以下のJSON形式のみで回答してください。
    【出力フォーマット】
...
with st.sidebar:
    ...
    if portfolio_items:
        ...
        if col1.button("修正"):
            ...
            st.rerun()
        if col2.button("復元", type="primary"):
    ...
    # ここでEnterキーによる改行が可能です
    new_reminder = st.text_area("内容を編集", value=st.session_state.reminder_text, height=150)
...
if st.session_state.events:
    ...
    for i, event in enumerate(st.session_state.events):
        ...
        cols[i].metric(event['name'], event['date'], f"あと {days_left} 日")
...
for i, (key, info) in enumerate(st.session_state.portfolio.items()):
    ...
    if p_data:
        ...
        if info['shares'] > 0:
            ...
            rows.append({
                "No.": i + 1, "銘柄": key, "数量": info['shares'],
                "取得単価": f"{info['cost']:,}", "現在値": f"{cur:,.2f} {day_change}",
                "損益(円)": f"¥{p_jpy:,.0f}"
            })

m_col1, m_col2 = st.columns(2)
m_col1.metric("総合計損益 (JPY)", f"¥{total_profit_jpy:,.0f}", delta=f"USD/JPY: {rate:.2f}")
m_col2.metric("米国株合計損益 (USD)", f"${total_profit_usd:,.2f}")
