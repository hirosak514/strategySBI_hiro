...
# --- 3. 解析・価格取得関数 ---
# キャッシュ有効期限を短く設定し、更新ボタンでクリアできるように設定
...
def get_live_prices(portfolio_keys):
    ...
    for key in portfolio_keys:
        ...
        try:
            ...
            # periodを1d、intervalを1mにすることで最新値を強制取得
            hist = stock.history(period="1d", interval="1m")
            if not hist.empty:
                prices[key] = {
                    "current": hist['Close'].iloc[-1],
                    "prev_close": hist['Close'].iloc[0] if len(hist) >= 2 else None
                }
            else:
                # 1分足が取れない場合は5日間から取得
                hist_backup = stock.history(period="5d")
                prices[key] = {
                    "current": hist_backup['Close'].iloc[-1],
                    "prev_close": hist_backup['Close'].iloc[-2] if len(hist_backup) >= 2 else None
                }
        except:
            prices[key] = None
